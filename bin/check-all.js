...
// ==========================================
console.log(`${colors.bright}👉 Stage 1/4: Running TypeScript Check...${colors.reset}`);
const tsStart = Date.now();
try {
  // Check if bin/check-ts.js exists, else fallback to tsc directly
  const checkTsPath = path.join(__dirname, "check-ts.js");
  let command = fs.existsSync(checkTsPath) ? "node bin/check-ts.js" : "npx tsc --noEmit --pretty false";
...
try {
  const stdout = execSync("npx eslint src/ --ext .ts,.js --quiet", { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] });
...
if (fs.existsSync(envPath)) {
  ...
  logStatus("WARN", "No .env file found in workspace root. Please ensure env vars are set.");
}
...
try {
  ...
  const stdout = execSync(buildCmd, { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] });
...
const allPassed = results.typeScript.passed && results.eslint.passed && results.build.passed;

logHeader(`📊 FINAL REPORT SUMMARY (Completed in ${totalDuration}s)`);

console.log(`  1. TypeScript Check : ${results.typeScript.passed ? "✅ PASSED" : "❌ FAILED"} (${results.typeScript.time}s)`);
console.log(`  2. ESLint Check     : ${results.eslint.passed ? "✅ PASSED" : "❌ FAILED"} (${results.eslint.time}s)`);
console.log(`  3. Environment Check: ${results.environment.passed ? "✅ PASSED" : "⚠️ WARN"} (${results.environment.time}s)`);
console.log(`  4. Project Build    : ${results.build.passed ? "✅ PASSED" : "❌ FAILED"} (${results.build.time}s)`);
...
if (allPassed) {
  ...
  if (!results.eslint.passed) {
    console.log(`#### ❌ 2. ESLINT VIOLATIONS:\n\`\`\`text\n${results.eslint.output.trim()}\n\`\`\`\n`);
  }
  if (!results.build.passed) {
    console.log(`#### ❌ 3. BUILD FAILURE LOGS:\n\`\`\`text\n${results.build.output.trim()}\n\`\`\`\n`);
  }

  console.log(`**INSTRUCTION FOR AGENT**: Please resolve all the errors listed above starting from TypeScript down to Build failures. Fix 1 file at a time and re-verify with \`node bin/check-all.js\`.`);
