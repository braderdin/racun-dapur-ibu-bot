...
function extractEnvironmentFromDevVars() {
  try {
    ...
    console.log(
      "🔗 DIRECT_URL: " + process.env.DIRECT_URL.substring(0, 50) + "...",
    );
  } catch (error) {
    console.error(
      "FAIL Failed to load environment from .dev.vars:",
      error.message,
    );
    throw error;
  }
...
async function runMigrations() {
  try {
    ...
    if (!directUrl) {
      throw new Error("FAIL DIRECT_URL environment variable is required!");
    }

    // Create a simple SQL statement to test connection
    console.log("RETRY Testing database connection...");
    ...
    console.log(
      "[Migration] Found " + sqlFiles.length + " migration files to execute...",
    );

    // For now, just copy the .dev.vars content to process.env
    // then call the TypeScript migration script using tsx
    console.log(
      "RETRY Running migration script with extracted environment variables...",
    );
    ...
    if (error.message.includes("DIRECT_URL")) {
      ...
      console.log(
        "\nE2E Current .dev.vars DIRECT_URL:",
        process.env.DIRECT_URL ? "is set" : "is NOT set",
      );
    }
