...
try {
  ...
  console.error("❌ Package 'twitter-api-v2' tidak dijumpai. Sila jalankan: npm install twitter-api-v2");
  process.exit(1);
}
...
try {
  ...
  console.error("❌ Package 'dotenv' tidak dijumpai. Sila jalankan: npm install dotenv");
  process.exit(1);
}
...
function logStatus(status, text) {
  if (status === "PASS") {
    console.log(`  ✅ ${colors.green}${colors.bright}[PASSED]${colors.reset} ${text}`);
  } else if (status === "WARN") {
    console.log(`  ⚠️  ${colors.yellow}${colors.bright}[WARNING]${colors.reset} ${text}`);
  } else {
    console.log(`  ❌ ${colors.red}${colors.bright}[FAILED]${colors.reset} ${text}`);
  }
}
...
function analyzeTwitterError(err) {
  ...
  if (status === 401 || message.includes("401")) {
    rootCause = "HTTP 401 Unauthorized - Kunci Pengesahan Tidak Sah / Terbatal.";
    actionRequired =
    ...
    rootCause = "HTTP 403 Forbidden - Kebenaran Akaun / App Permission Tidak Mencukupi.";
    actionRequired =
    ...
    rootCause = "HTTP 429 Too Many Requests - Had Kadar Request (Rate Limit) Telah Dicapai.";
    actionRequired = "Tunggu 15 minit sebelum mencuba semula. X API v2 mengenakan had kuota yang ketat untuk Free Tier.";
  } else if (message.includes("ENOTFOUND") || message.includes("ETIMEDOUT")) {
    rootCause = "Ralat Rangkaian / DNS Timeout.";
    actionRequired = "Semak sambungan internet atau tetapan sambungan proxy/VPN anda.";
  }
...
async function runTwitterDiagnosticSuite() {
  ...
  // ==========================================
  console.log(`${colors.bright}👉 Stage 1/5: Menyemak Kewujudan Kunci Rahsia X (Twitter)...${colors.reset}`);

  const apiKey = process.env.X_API_KEY || process.env.X_Consumer_Key;
  const apiSecret = process.env.X_API_KEY_SECRET || process.env.X_Consumer_Key_Secret;
  ...
  if (missingKeys.length === 0) {
    ...
    recordResult("1. Key Presence", "FAIL", `Missing: ${missingKeys.join(", ")}`);
  }

  if (bearerToken) {
    console.log(`     📌 Bearer Token   : ${bearerToken.substring(0, 15)}... (OAuth 2.0 App-Only)`);
  } else {
  ...
  try {
    ...
    if (v1User && v1User.screen_name) {
      logStatus("PASS", `Sambungan v1.1 Berjaya! Akaun Terhubung: @${v1User.screen_name} (ID: ${v1User.id_str})`);
  ...
  try {
    ...
    if (v2User && v2User.data) {
      logStatus("PASS", `Sambungan v2 User Context Berjaya! User: @${v2User.data.username}`);
      if (v2User.data.public_metrics) {
        console.log(`     📊 Pengikut       : ${v2User.data.public_metrics.followers_count} | Mengikut: ${v2User.data.public_metrics.following_count}`);
      }
  ...
  if (bearerToken) {
    try {
      ...
      const sampleSearch = await appOnlyClient.v2.get("tweets/sample/stream", {}, { fullResponse: false }).catch(() => null);
      ...
      const analysis = analyzeTwitterError(err);
      logStatus("WARN", `Sistem Bearer Token Memberi Respons: ${analysis.rootCause}`);
      recordResult("4. Bearer Token App-Only", "WARN", analysis.message, analysis);
    }
  } else {
  ...
  try {
    ...
    if (userMe && userMe.data) {
      logStatus("PASS", "Kebenaran Akaun & Akses Token Disahkan Sedia Untuk Hantaran Tweet!");
      recordResult("5. Write Permissions Audit", "PASS", "Verified");
    }
  } catch (err) {
    const analysis = analyzeTwitterError(err);
    logStatus("FAIL", `Kegagalan Kebenaran Akses: ${analysis.rootCause}`);
    recordResult("5. Write Permissions Audit", "FAIL", analysis.message, analysis);
  }
...
function printFinalSummary(startTime) {
  ...
  testResults.forEach((r) => {
    ...
    console.log(`  ${icon} ${r.stage.padEnd(28)} : ${r.status.padEnd(6)} [${r.details}]`);
  });
  ...
  if (allPassed) {
    ...
    console.log(`${colors.green}    Bot sedia untuk membuat hantaran Thread Tweet di persekitaran pengeluaran!${colors.reset}\n`);
    process.exit(0);
  } else {
    console.log(`\n${colors.red}${colors.bright} ⚠️ ATTENTION: RALAT DETEKSI DALAM KREDENTIAL X (TWITTER)!${colors.reset}`);
    console.log(`${colors.red}    Sila salin laporan penuh di bawah untuk diserahkan kepada Gemini / AI Agent:${colors.reset}\n`);

    console.log(`${colors.yellow}=================== COPY START (FOR GEMINI / AI AGENT) ===================${colors.reset}`);
    ...
    console.log(`**INSTRUCTION FOR AGENT**: Please resolve the Twitter OAuth credentials issues above. Update .env.local or guide Chip Besar on fixing App Permissions in X Developer Portal.`);
    console.log(`${colors.yellow}==================== COPY END (FOR GEMINI / AI AGENT) ====================${colors.reset}\n`);
...
runTwitterDiagnosticSuite().catch((err) => {
  console.error("Ralat Parah semasa menjalankan ujian diagnostik Twitter:", err);
  process.exit(1);
});