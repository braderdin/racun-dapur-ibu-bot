...
function loadEnvFromDevVars() {
  try {
    ...
    if (!DIRECT_URL || !SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
      ...
      const devVarsContent = fs.readFileSync(
        path.join(process.cwd(), ".dev.vars"),
        "utf8",
      );
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (statements.length === 0) {
        console.log(`WARN  No valid SQL statements found in ${fileName}`);
        resolve({ fileName, statementsExecuted: 0, statementsFailed: 0 });
        return;
      }

      console.log(
        `REPORT Found ${statements.length} SQL statements in ${fileName}`,
      );
      ...
      async function executeStatements() {
        for (let i = 0; i < statements.length; i++) {
          ...
          try {
            console.log(
              `  [${stmtNumber}/${statements.length}] Executing: ${stmt.substring(0, 100)}...`,
            );
            ...
            console.log(`  OK2 Statement ${stmtNumber} successful`);
          } catch (stmtError) {
            console.error(
              `  FAIL Statement ${stmtNumber} failed: ${stmtError.message}`,
            );
            statementsFailed++;
            migrationStats.failedStatements++;

            // Continue with next statements but mark file as failed
            console.log(`  WARN  Continuing with remaining statements...`);
          }
        }

        await client.end();
        console.log(
          `\nOK2 Successfully executed ${statementsExecuted} statements from ${fileName}`,
        );
        console.log("=".repeat(60));
      ...
      console.error(
        "[Migration] Failed to execute " + fileName + ": " + error.message,
      );
      reject(error);
    }
  });
...
async function main() {
  ...
  try {
    console.log(
      "[Migration] Automated Database Migration Helper Script started",
    );
    console.log("[Migration] Directory: " + migrationsDir);
    console.log(
      "[Migration] Started at: " +
        new Date(migrationStats.startTime).toISOString(),
    );
    ...
    console.log(
      "\n[Migration] Found " + sqlFiles.length + " migration file(s):",
    );
    ...
    for (let i = 0; i < sqlFiles.length; i++) {
      ...
      console.log(
        "\n[Migration] Executing migration file " +
          (i + 1) +
          "/" +
          sqlFiles.length +
          ": " +
      ...
      try {
        ...
        console.error(
          "[Migration] File " + file + " failed completely: " + error.message,
        );
        migrationStats.failedFiles++;

        // Continue with remaining files unless critical error
        if (i < sqlFiles.length - 1) {
          console.log("WARN  Continuing with remaining migration files...");
        } else {
    ...
    migrationStats.executionTime =
      migrationStats.endTime - migrationStats.startTime;

    console.log("\n" + "=".repeat(80));
    console.log("[Migration] Migration execution completed!");
    ...
    console.log(
      "  Successful Statements: " + migrationStats.successfulStatements,
    );
    console.log("  Failed Statements: " + migrationStats.failedStatements);
    console.log(
      "  Execution Time: " + migrationStats.executionTime / 1000 + " seconds",
    );
    console.log(
      "  Completed at: " + new Date(migrationStats.endTime).toISOString(),
    );

    // Verify migration success
    if (migrationStats.failedFiles > 0) {
      console.warn(
        "[Migration] Some migration files failed. Please check the logs above for details.",
      );
      process.exit(1);
    }

    console.log("OK2 All migrations executed successfully!");
  } catch (error) {
    migrationStats.endTime = Date.now();
    migrationStats.executionTime =
      migrationStats.endTime - migrationStats.startTime;
    ...
    console.log("[Migration] Execution Summary (Partial):");
    console.log(
      "  Start Time: " + new Date(migrationStats.startTime).toISOString(),
    );
    console.log(
      "  End Time: " + new Date(migrationStats.endTime).toISOString(),
    );
    console.log(
      "  Execution Time: " + migrationStats.executionTime / 1000 + " seconds",
    );
...
const timeoutPromise = new Promise((_, reject) => {
  setTimeout(
    () =>
      reject(
        new Error("Migration timed out after 30 seconds - aborting for safety"),
      ),
    30000,
  );
});
...
Promise.race([main(), timeoutPromise])
  ...
  .catch((error) => {
    console.error("BANG Fatal error during migration:", error.message);
    process.exit(1);
  });
...
process.on("uncaughtException", (error) => {
  console.error("BANG Uncaught exception:", error);
  console.log("TOOL Attempting graceful shutdown...");
  setTimeout(() => process.exit(1), 5000).unref();
});

process.on("unhandledRejection", (reason, promise) => {
  console.error("BANG Unhandled rejection at:", promise, "reason:", reason);
  console.log("TOOL Attempting graceful shutdown...");
  setTimeout(() => process.exit(1), 5000).unref();
});
