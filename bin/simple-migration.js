...
function loadEnvFromDevVars() {
  try {
    ...
    if (!fs.existsSync(devVarsPath)) {
      console.error("FAIL ERROR: .dev.vars file not found!");
      console.log(
        "   Please ensure the .dev.vars file exists in the project root.",
      );
      process.exit(1);
    }
    ...
    if (!directUrl) {
      console.error("FAIL ERROR: DIRECT_URL not found in .dev.vars file!");
      ...
      console.log("\nTOOL Please add DIRECT_URL to .dev.vars file:");
      console.log(
        '   DIRECT_URL="postgresql://username:password@your-supabase-url:5432/postgres?pgbouncer=false"',
      );
      process.exit(1);
    }
    ...
    console.error("FAIL ERROR loading .dev.vars:", error.message);
    process.exit(1);
  }
}
...
async function executeSql(conn, sql) {
  try {
    ...
    console.error("FAIL SQL Error:", error.message);
...
async function migrate() {
  console.log("ROCKET Starting Phase 3 Database Migration...");
  console.log("=============================================================");
  ...
  try {
    console.log("RETRY Connecting to Supabase database...");
    await client.connect();
    console.log("OK2 Connected successfully");
    ...
    if (files.length === 0) {
      console.log("WARN  No migration files found in", migrationsDir);
      await client.end();
      return;
    }
    ...
    for (const file of files) {
      ...
      if (statements.length === 0) {
        console.log("WARN  No valid SQL statements found in", file);
        continue;
      }

      console.log("   E2E Executing", statements.length, "SQL statements...");

      // Execute each statement
      for (let i = 0; i < statements.length; i++) {
        const stmt = statements[i];
        console.log(
          "      " + (i + 1).toString().padStart(2) + "/" + statements.length,
        );

        try {
          await client.query(stmt);
          console.log("         OK2 Executed");
        } catch (error) {
    ...
    console.log("\nOK2 Phase 3 database deployment completed successfully!");
    console.log("\n📋 Created schemas:");
    console.log(
      "   • posted_products (with dual-engine support for Lazada & Shopee)",
    );
    console.log(
      "   • link_clicks (for tracking shortlink clicks and conversions)",
    );
    console.log(
      "\nROCKET Next steps: Continue with Phase 4 implementations...",
    );
  } catch (error) {
    console.error("\nFAIL MIGRATION FAILED:", error.message);
    console.error("\nTOOL Possible causes:");
