...
export class FullEcosystemTest {
  ...
  async testServiceInitialization(): Promise<any[]> {
    ...
    results.push({
      test: 'Supabase Client Initialization',
      description: 'Verify Supabase client can be created with environment variables',
      passed: supabaseInitialized,
      details: supabaseInitialized ? '✅ Client initialized successfully' : '❌ Missing Supabase environment variables',
    });
    ...
    try {
      ...
      results.push({
        test: 'Catalog Service Initialization',
        description: 'Verify Catalog service can fetch preview data from database',
        passed: true,
    ...
    try {
      ...
      results.push({
        ...
        passed: healthCheck,
        details: healthCheck ? '✅ Realtime service healthy' : '❌ Realtime service failed health check',
      });
    } catch (error) {
      results.push({
        test: 'Realtime Feed Service Initialization',
    ...
    try {
      ...
      const categorized = this.budgetFilterService.categorizeByBudget(testProducts);
      const totalCategorized = Object.values(categorized).reduce((sum, arr) => sum + arr.length, 0);
      results.push({
        test: 'Budget Filter Service Initialization',
        description: 'Verify Budget filter service can categorize products by price ranges',
    ...
    try {
      ...
      results.push({
        test: 'Dual Buy Analytics Service Initialization',
        description: 'Verify Dual buy analytics service can track and process events',
        passed: testAnalytics,
        details: testAnalytics ? '✅ Analytics service working' : '❌ Analytics service test failed',
      });
    } catch (error) {
      results.push({
        test: 'Dual Buy Analytics Service Initialization',
  ...
  async testDatabaseIntegration(): Promise<any[]> {
    ...
    try {
      const { data, error } = await this.supabase
        .from('posted_products')
        .select('id, product_name, category, lazada_availability, shopee_availability')
        .limit(5);
      ...
      const availableProducts = data.filter(p => 
        p.lazada_availability === 'available' || p.shopee_availability === 'available'
      );

      results.push({
        test: 'RLS Policy Configuration',
        description: 'Verify Row Level Security allows public reads of available products',
    ...
    try {
      ...
      results.push({
        ...
        description: 'Verify Full-Text Search index is available for Malay/English search',
        passed: ftsAvailable,
        details: ftsAvailable ? '✅ FTS configuration available' : '❌ FTS configuration not available',
      });
    } catch (error) {
      results.push({
        test: 'Full-Text Search Configuration',
  ...
  async testRealtimeFeatures(): Promise<any[]> {
    ...
    try {
      ...
      results.push({
        ...
        passed: healthCheck,
        details: healthCheck ? '✅ Realtime service healthy' : '❌ Realtime service unhealthy',
      });
    } catch (error) {
      results.push({
        test: 'Realtime Service Health',
    ...
    try {
      ...
      if (testProduct.length > 0) {
        const countdown = this.flashSaleService.getFlashSaleCountdown(testProduct[0]);
        const formatted = this.flashSaleService.formatCountdown(countdown);

        results.push({
          test: 'Flash Sale Calculations',
          description: 'Verify Flash sale service can calculate and format countdown timers',
  ...
  async testAnalyticsProcessing(): Promise<any[]> {
    ...
    try {
      const dashboardData = await this.dualBuyAnalyticsService.getDashboardAnalytics();
    ...
    try {
      ...
      results.push({
        ...
        passed: simulatedClickData && simulatedClickData.productId === 'test-product-123',
    ...
    try {
      ...
      const analytics = this.budgetFilterService.generateFilterAnalytics(testProducts, '<20');
  ...
  async testSearchPerformance(): Promise<any[]> {
    ...
    try {
      ...
      results.push({
        ...
        description: 'Verify Catalog service can perform searches within acceptable time',
  ...
  async testRealDealWorkflow(): Promise<any[]> {
    ...
    try {
      ...
      const isFlashProduct = flashSales.some(fs => fs.productId === testProduct.id);

      results.push({
        test: 'Deal Discovery Workflow',
        description: 'Verify system can discover available deals and flash sales',
        passed: true,
    ...
    try {
      ...
      const cheapestCategory = Object.entries(categorized).find(([_, products]) => products.length > 0);
    ...
    try {
      ...
      const analytics = await this.dualBuyAnalyticsService.getClickAnalytics(testProduct.id, {
      ...
      results.push({
        ...
        description: 'Verify system can retrieve and process click analytics for products',
    ...
    try {
      ...
      const hasRelevantResults = searchResults.data.some(product =>
