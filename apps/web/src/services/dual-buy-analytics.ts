...
export class DualBuyAnalyticsService {
  ...
  private async incrementProductClickCount(productId: string): Promise<void> {
    try {
      ...
      const { error } = await this.supabase.rpc("increment_total_clicks", {
        product_id: productId,
      });
  ...
  async getClickAnalytics(
    ...
    const clicksByPlatform = data.reduce((acc: any, click) => {
      ...
      if (
        new Date(click.timestamp) > new Date(acc[click.platform].lastClickAt)
      ) {
    ...
    const result: ClickAnalyticsData = {
      productId,
      platform:
        (Object.keys(clicksByPlatform)[0] as "lazada" | "shopee") || "lazada",
    ...
    if (product) {
      result.affiliateCode =
        product.platform === "lazada"
          ? product.lazada_affiliate_code
          : product.platform === "shopee"
            ? product.shopee_affiliate_code
            : undefined;
    }
  ...
  async getDashboardAnalytics(): Promise<any> {
    ...
    return {
      metrics: {
        ...
        uniqueProductsToday: [...new Set(todayClicks?.map((c) => c.product_id))]
          .length,
        platformsToday: {
          lazada:
            todayClicks?.filter((c) => c.platform === "lazada").length || 0,
          shopee:
            todayClicks?.filter((c) => c.platform === "shopee").length || 0,
        },
      },
