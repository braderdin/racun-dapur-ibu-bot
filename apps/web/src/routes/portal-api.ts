...
function handleApiError(error: any, operation: string): NextResponse {
  ...
  return NextResponse.json(
    { error: 'Internal server error', message: `An error occurred during ${operation}` },
    { status: 500 }
  );
}
  ...
  try {
    ...
    const params: SearchParams = {
      ...
      filters: {
        ...
        budget: {
          min: searchParams.get('budgetMin') ? parseFloat(searchParams.get('budgetMin')!) : undefined,
          max: searchParams.get('budgetMax') ? parseFloat(searchParams.get('budgetMax')!) : undefined,
        },
      },
    };
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const requiredFields = ['id', 'sku', 'product_name', 'category', 'lazada_url', 'shopee_url'];
    ...
    if (error) {
      return NextResponse.json(
        { error: 'Database error', message: `Failed to create product: ${error.message}` },
        { status: 500 }
      );
    }
    ...
    endpoints: [
      {
        ...
        parameters: [
          { name: 'query', type: 'string', optional: true, description: 'Search keywords' },
          { name: 'category', type: 'string', optional: true, description: 'Product category' },
          { name: 'discountMin', type: 'number', optional: true, description: 'Minimum discount percentage' },
          { name: 'budgetMin', type: 'number', optional: true, description: 'Minimum budget' },
          { name: 'budgetMax', type: 'number', optional: true, description: 'Maximum budget' },
          { name: 'limit', type: 'number', optional: true, default: 20, description: 'Results per page' },
          { name: 'offset', type: 'number', optional: true, default: 0, description: 'Page offset' },
        ],
      },
      {
        ...
        parameters: [
          { name: 'q', type: 'string', optional: false, description: 'Search query' },
          { name: 'limit', type: 'number', optional: true, default: 20, description: 'Results limit' },
        ],
      },
      {
        ...
        parameters: [
          { name: 'limit', type: 'number', optional: true, default: 10, description: 'Number of flash sales to return' },
        ],
      },
      {
