...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (event.event_type === 'dual_buy_click') {
      await processDualBuyClickEvent(event.data as DualBuyClickEvent, analyticsRecord.batch_id);
    }
...
async function processDualBuyClickEvent(
  ...
  try {
    ...
    if (productId && Math.random() < 0.1) { // Simulate 10% chance for promotion
...
export async function BATCH_POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!Array.isArray(body)) {
      return NextResponse.json(
        { error: 'Invalid batch', message: 'Request body must be an array of events' },
        { status: 400 }
      );
    }
    ...
    if (invalidEvents.length > 0) {
      return NextResponse.json(
        { 
          ...
          details: invalidEvents.flatMap((result, index) => 
            result.error.errors.map(err => `Event ${index + 1}: ${err.path.join('.')}: ${err.message}`)
          ).join(', ')
        },
        { status: 400 }
      );
    ...
    const dualBuyEvents = events.filter(event => event.event_type === 'dual_buy_click');
