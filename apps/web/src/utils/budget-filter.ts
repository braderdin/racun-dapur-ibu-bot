...
export class BudgetFilterService {
  ...
  filterWithStats(
    ...
    const rangeFilters = budgetsArray.map((budget) =>
      this.getBudgetConfig(budget),
    );
    ...
    return products.filter((product) => {
      ...
      switch (platform) {
        case "lazada":
          return (
            lazadaPrice !== null &&
            lazadaPrice !== undefined &&
            lazadaPrice >= minPrice &&
            lazadaPrice <= maxPrice
          );
        ...
        case "shopee":
          return (
            ...
            shopeePrice >= minPrice &&
            shopeePrice <= maxPrice
          );
    ...
    switch (platform) {
      case "lazada":
        return products.filter(
          (p) => p.lazada_discount && p.lazada_discount >= minDiscount,
        );

      case "shopee":
        return products.filter(
          (p) => p.shopee_discount && p.shopee_discount >= minDiscount,
        );
  ...
  calculatePriceDistribution(products: CatalogProduct[]): {
    ...
    products.forEach((product) => {
      ...
      const prices = [lazadaPrice, shopeePrice].filter(
  ...
  private calculateBudgetStats(filtered: CatalogProduct[]): BudgetStats {
    ...
    const mostExpensive = {
      ...
      productId: filtered.find(
        (p) =>
          p.lazada_price === Math.max(...prices) ||
          p.shopee_price === Math.max(...prices),
      )?.id,
    };
  ...
  getLocalizedBudgetLabels(
    language: "ms" | "en" = "ms",
  ): Record<BudgetRange, string> {
  ...
  generateFilterAnalytics(
    ...
    const averagePrice =
      filtered.reduce((sum, product) => {
        const prices = this.getAvailablePrices(product);
        return (
          sum + prices.reduce((pSum, price) => pSum + price, 0) / prices.length
        );
      }, 0) / (count || 1);
  ...
  validateBudgetConfig(budget: any): budget is BudgetFilter {
    return (
      ...
      (budget.applyToBothPlatforms === undefined ||
        typeof budget.applyToBothPlatforms === "boolean")
    );
  }
    ...
    this.getAllBudgetRanges().forEach((budget) => {
      ...
      const filtered = products.filter(
        (product) =>
          ...
          this.getAvailablePrices(product).some(
            (price) => price <= (budgetConfig.max || Infinity),
          ),
      );
  ...
  debugBudgetFilter(products: CatalogProduct[]): {
    ...
    priceDistribution: ReturnType<
      BudgetFilterService["calculatePriceDistribution"]
    >;
    averagePrice: number;
  } {
    const categorized = this.categorizeByBudget(products);
    const categoryCounts = Object.entries(categorized).reduce(
      (acc, [key, value]) => {
