...
export class AIFallbackEngine {
  ...
  async generateCopy(product: ProductItem, agentId?: string): Promise<GeneratedCopy> {
    ...
    console.log(`🎯 Using fallback strategy with ${this.config.maxRetries} retries max`);
    ...
    try {
      ...
      throw new Error(`AI generation failed after all fallback attempts: ${error.message}`);
    }
  }
  ...
  async generateMultipleCopies(
    ...
    console.log(`🎯 Generating copy for ${Math.min(limit, products.length)} products...`);
...
class MockOpenRouterService implements OpenRouterService {
  async generateCopy(product: ProductItem): Promise<GeneratedCopy> {
    ...
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, CONSTANTS.OPENROUTER_DELAY_MS || 3000));
  ...
  private generateCreativeCopy(product: ProductItem, source: string): Omit<GeneratedCopy, 'platform' | 'confidence' | 'fallbackChainUsed'> {
    const hooks = [
    ...
    const threadTarget: "single-tweet" | "thread-2" = bodies.length > 1 ? "thread-2" : "single-tweet";
...
class HeuristicRuleEngine implements HeuristicRuleEngine {
  async generateCopy(product: ProductItem): Promise<GeneratedCopy> {
    console.log("🔧 [Mock Heuristic] Generating copy using rule-based engine...");
  ...
  private generateCreativeCopy(product: ProductItem): Omit<GeneratedCopy, 'platform' | 'confidence' | 'fallbackChainUsed'> {
    const hooks = [
...
const aiFallbackEngine = new AIFallbackEngine(openRouterService, geminiService, heuristicService);
