...
export class OpenRouterAIService {
  ...
  /**
   * Hasilkan ayat "Racun Dapur Ibu" berpandukan Formula Psikologi (Masalah -> Penyelesaian -> Diskaun)
   */
  async generateCopywriting(product: ProductItem): Promise<GeneratedCopy> {
    // WAJIB: Enforce Delay 3 Saat untuk menjaga had OpenRouter Free Tier
    await new Promise((resolve) => setTimeout(resolve, CONSTANTS.OPENROUTER_DELAY_MS));
...
`;
    ...
    try {
      ...
      console.error("OpenRouter AI Error, menggunakan fallback copywriting:", error);
      // Fallback Heuristic Copywriting jika AI timeout
      return {
