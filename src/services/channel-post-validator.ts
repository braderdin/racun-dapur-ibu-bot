...
const xTweet1Schema = z.object({
  content: z
    ...
    .regex(
      /^[^\x00-\x1F\x7F]*$/,
      "Tweet 1 content contains control characters",
    ),
...
const xTweet2Schema = z.object({
  content: z
    ...
    .regex(
      /^[^\x00-\x1F\x7F]*$/,
      "Tweet 2 content contains control characters",
    ),
...
export class ChannelPostValidator {
  ...
  validateContent<T extends AnyContent>(
    content: T,
    platform: string,
  ): {
    success: boolean;
  ...
  private performAdditionalValidations(
    ...
    if (content.platform === PLATFORM_TYPE.X) {
      this.validateXTweetContent(
        content as ValidatedXContent,
        config,
        warnings,
      );
    } else if (content.platform === PLATFORM_TYPE.FACEBOOK) {
      this.validateFBCustomerPost(
        content as ValidatedFBContent,
        config,
        warnings,
      );
    }
  ...
  private validateXTweetContent(
    ...
    if (!content.tweet1.isHook) {
      warnings.push(
        "Tweet 1 is marked as not being a hook - may reduce engagement",
      );
    }
    ...
    if (!content.tweet2.affiliateUrl) {
      warnings.push(
        "Tweet 2 missing affiliate link - reduces conversion potential",
      );
    }

    // Validate hashtag presence
    const combinedText =
      `${content.tweet1.content} ${content.tweet2.content}`.toLowerCase();
    ...
    if (!hashtagsInContent) {
      warnings.push(
        `Content should include brand hashtags: ${config.hashtags}`,
      );
    }
  }
  ...
  private validateFBCustomerPost(
    ...
    if (!content.post.storytelling) {
      warnings.push(
        "Post should be in storytelling format for better engagement",
      );
    }
    ...
    const hasUrgency = config.urgencyWords.some((word) =>
      postText.includes(word.toLowerCase()),
    );
    if (!hasUrgency) {
      warnings.push(
        `Post should include urgency words: ${config.urgencyWords.join(", ")}`,
      );
    }
  }
